...
"""Fuzz testing: complete point centroid implosion modification on subdivision mesh in USD stage."""
...
def test_mesh_point_implosion_center():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_mesh_point_implosion_center_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_mesh_point_implosion_center_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Centroid implosion
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  points = np.array(mesh.GetPointsAttr().Get()) * 0.05
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_mesh_point_implosion_center_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_mesh_point_implosion_center_modified_usd.png'
  )
  test_helpers.robust_assert_close(
