...
"""Fuzz testing: tight narrow cone angle searchlight modification on USD stage."""
...
def test_spot_light_tight_searchlight():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_spot_light_tight_searchlight_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_spot_light_tight_searchlight_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set tight searchlight beam
  spot_light = UsdLux.SphereLight.Get(stage, '/root/Area/Area')
  shaping = UsdLux.ShapingAPI(spot_light.GetPrim())
  shaping.GetShapingConeAngleAttr().Set(5.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_spot_light_tight_searchlight_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_spot_light_tight_searchlight_modified_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_modified[..., :3], image_usd_modified, atol=0.25
  )
