...
"""Fuzz testing: simultaneous compound modifications on USD stage."""
...
def test_compound_modifications():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_compound_modifications_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_compound_modifications_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.05
  )

  # Modification: Compound changes across multiple prims
  # 1. Transform cube
  cube_xform = UsdGeom.Xformable.Get(stage, '/root/Cube')
  cube_xform.AddTranslateOp(opSuffix='fuzz_trans').Set((0.1, -0.1, 0.1))

  # 2. Modify mesh points
  ...
  # 4. Modify light intensity
  light = UsdLux.SphereLight.Get(stage, '/root/Light/Light')
  light.GetIntensityAttr().Set(450.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_compound_modifications_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_compound_modifications_modified_usd.png'
  )
  test_helpers.robust_assert_close(
