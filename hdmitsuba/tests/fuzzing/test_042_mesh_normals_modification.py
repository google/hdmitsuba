...
"""Fuzz testing: mesh normals modification on USD stage."""
...
def test_normals_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_normals_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_normals_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: update normals and points
  mesh = UsdGeom.Mesh.Get(stage, '/root/Plane/Plane')
  normals = [
      Gf.Vec3f(0.1, 0.1, 0.9),
      Gf.Vec3f(0.1, 0.1, 0.9),
      Gf.Vec3f(0.1, 0.1, 0.9),
      Gf.Vec3f(0.1, 0.1, 0.9),
      Gf.Vec3f(0.1, 0.1, 0.9),
  ...
  points = np.array(mesh.GetPointsAttr().Get()) * 0.9
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_normals_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_normals_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
