...
"""Fuzz testing: non-uniform extreme scale modification on RectLight in USD stage."""
...
def test_rect_extreme_scale():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_rect_extreme_scale_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_rect_extreme_scale_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: non-uniform scaling on Area light Xform
  rect_xform = UsdGeom.Xformable.Get(stage, '/root/Area')
  rect_xform.AddScaleOp(opSuffix='fuzz_scale').Set((5.0, 0.1, 1.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_rect_extreme_scale_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_rect_extreme_scale_modified_usd.png'
  )
  test_helpers.robust_assert_close(
