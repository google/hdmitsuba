...
"""Fuzz testing: active state toggle and material binding switching modification on USD stage."""
...
def test_active_inactivation_rebind():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_active_inactivation_rebind_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_active_inactivation_rebind_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Rebind subset materials then toggle light active state
  ...
  light = stage.GetPrimAtPath('/root/Light/Light')
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_active_inactivation_rebind_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_active_inactivation_rebind_modified_usd.png'
  )
  test_helpers.robust_assert_close(
