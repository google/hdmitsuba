...
"""Fuzz testing: geom subset material unbinding on USD stage."""
...
def test_subsets_disable():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_subsets_disable_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_subsets_disable_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: unbind material on red subset
  red_subset = stage.GetPrimAtPath('/root/Cube/Cube/red')
  UsdShade.MaterialBindingAPI(red_subset).UnbindAllBindings()
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_subsets_disable_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_subsets_disable_modified_usd.png'
  )
  test_helpers.robust_assert_close(
