...
"""Fuzz testing: explicit normals inversion modification on USD stage."""
...
def test_mesh_flip_normals():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_mesh_flip_normals_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_mesh_flip_normals_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Invert all normals
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  normals = np.array(mesh.GetNormalsAttr().Get()) * -1.0
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_mesh_flip_normals_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_mesh_flip_normals_modified_usd.png'
  )
  test_helpers.robust_assert_close(
