...
"""Fuzz testing: camera sensor reposition modification on IrradianceMeter in USD stage."""
...
def test_irradiancemeter_reposition():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_irradiancemeter_reposition_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_irradiancemeter_reposition_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: reposition camera sensor closer to light
  cam_xform = UsdGeom.Xformable.Get(stage, '/root/Camera')
  cam_xform.AddTranslateOp(opSuffix='fuzz_shift').Set((0.5, 0.0, -1.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_irradiancemeter_reposition_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_irradiancemeter_reposition_modified_usd.png'
  )
  test_helpers.robust_assert_close(
