...
"""Fuzz testing: curve widths modification on USD stage."""
...
def test_curve_width_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_curve_width_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_curve_width_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update curve widths
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  widths = np.array(curves.GetWidthsAttr().Get()) * 2.0
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_curve_width_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_curve_width_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
