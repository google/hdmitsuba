...
"""Fuzz testing: simultaneous camera transform and clipping modification on USD stage."""
...
def test_camera_shift_clipping():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_shift_clipping_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_shift_clipping_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: update camera transform and clippingRange concurrently
  cam_xform = UsdGeom.Xformable.Get(stage, '/root/Camera')
  cam_xform.AddTranslateOp(opSuffix='fuzz_move').Set((0.1, -0.1, 0.1))
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetClippingRangeAttr().Set(Gf.Vec2f(1.0, 80.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_shift_clipping_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_camera_shift_clipping_modified_usd.png'
  )
  test_helpers.robust_assert_close(
