...
"""Fuzz testing: material roughness modification on USD stage."""
...
def test_material_roughness_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_material_roughness_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_material_roughness_modification_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.05
  )

  # Modification: Set roughness
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/Material/Principled_BSDF'
  )
  shader.GetInput('roughness').Set(0.1)
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_material_roughness_modification_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_material_roughness_modification_modified_usd.png',
  )
  test_helpers.robust_assert_close(
