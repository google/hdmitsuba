...
"""Fuzz testing: collinear lighting occlusion eclipse modification on DiskLight in USD stage."""
...
def test_disk_light_collinear_eclipse():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/lights/disk.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_disk_light_collinear_eclipse_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_disk_light_collinear_eclipse_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Translate disk behind plane
  disk_xform = UsdGeom.Xformable.Get(stage, '/root/Area')
  disk_xform.AddTranslateOp(opSuffix='fuzz_eclipse').Set((0, 0, -2.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_disk_light_collinear_eclipse_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_disk_light_collinear_eclipse_modified_usd.png'
  )
  test_helpers.robust_assert_close(
