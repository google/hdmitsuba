...
"""Fuzz testing: multiple sequential transformations on USD stage."""
...
def test_multiple_transforms():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_multiple_transforms_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_multiple_transforms_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.05
  )

  # Modification: Compound sequential operations
  cube_xform = UsdGeom.Xformable.Get(stage, '/root/Cube')
  cube_xform.AddScaleOp(opSuffix='seq_scale').Set((0.8, 1.1, 0.9))
  cube_xform.AddRotateXYZOp(opSuffix='seq_rotate').Set((10.0, -10.0, 20.0))
  cube_xform.AddTranslateOp(opSuffix='seq_translate').Set((-0.1, 0.2, 0.1))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_multiple_transforms_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_multiple_transforms_modified_usd.png'
  )
  test_helpers.robust_assert_close(
