...
"""Fuzz testing: simultaneous camera forward motion and geometry inactivation modification on USD stage."""
...
def test_active_mesh_disappearing_act():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_active_mesh_disappearing_act_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_active_mesh_disappearing_act_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Inactivate cube and translate camera
  cube = stage.GetPrimAtPath('/root/Cube')
  cube.SetActive(False)

  cam_xform = UsdGeom.Xformable.Get(stage, '/root/Camera')
  cam_xform.AddTranslateOp(opSuffix='fuzz_forward').Set((0, 0, -1.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_active_mesh_disappearing_act_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_active_mesh_disappearing_act_modified_usd.png'
  )
  test_helpers.robust_assert_close(
