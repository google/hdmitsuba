...
"""Fuzz testing: uniform flat alignment modification on normal vectors in USD stage."""
...
def test_dynamic_normal_flattening():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/shapes/subdiv_plane.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_dynamic_normal_flattening_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_dynamic_normal_flattening_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set flat identical normals
  mesh = UsdGeom.Mesh.Get(stage, '/root/Plane/Plane')
  points_len = len(mesh.GetPointsAttr().Get())
  normals = [Gf.Vec3f(0, 0, 1)] * points_len
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_dynamic_normal_flattening_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_dynamic_normal_flattening_modified_usd.png'
  )
  test_helpers.robust_assert_close(
