...
"""Fuzz testing: camera transform modification on USD stage."""
...
def test_camera_transform_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_transform_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_transform_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.05
  )

  # Modification: Add translation offset to camera
  cam_xform = UsdGeom.Xformable.Get(stage, '/root/Camera')
  cam_xform.AddTranslateOp(opSuffix='fuzz_camera_move').Set((0.2, 0.2, 0.2))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_transform_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_camera_transform_modification_modified_usd.png',
  )
  test_helpers.robust_assert_close(
