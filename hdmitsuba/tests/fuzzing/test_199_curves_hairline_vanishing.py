...
"""Fuzz testing: hairline vanishing small widths modification on BasisCurves in USD stage."""
...
def test_curves_hairline_vanishing():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_curves_hairline_vanishing_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_curves_hairline_vanishing_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set hairline width
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  widths = np.array(curves.GetWidthsAttr().Get()) * 0.0001
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_curves_hairline_vanishing_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_curves_hairline_vanishing_modified_usd.png'
  )
  test_helpers.robust_assert_close(
