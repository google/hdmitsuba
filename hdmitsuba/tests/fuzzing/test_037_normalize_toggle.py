...
"""Fuzz testing: normalize attribute toggle on area light in USD stage."""
...
def test_normalize_toggle():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_normalize_toggle_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_normalize_toggle_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set normalize to False
  rect_light = UsdLux.RectLight.Get(stage, '/root/Area/Area')
  rect_light.GetNormalizeAttr().Set(False)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_normalize_toggle_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_normalize_toggle_modified_usd.png'
  )
  test_helpers.robust_assert_close(
