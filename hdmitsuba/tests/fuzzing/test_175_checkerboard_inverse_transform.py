...
"""Fuzz testing: inverse scaling matrix modification on checkerboard BSDF in USD stage."""
...
def test_checkerboard_inverse_transform():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_checkerboard_inverse_transform_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_checkerboard_inverse_transform_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set fractional inverse UV scale matrix
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/ground/myshader/nested_bsdf/base_color'
  )
  mat = Gf.Matrix3d(0.05, 0, 0, 0, 0.05, 0, 0, 0, 1.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_checkerboard_inverse_transform_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_checkerboard_inverse_transform_modified_usd.png',
  )
  test_helpers.robust_assert_close(
