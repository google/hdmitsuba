...
"""Fuzz testing: scale modification on USD stage."""
...
def test_scale_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_scale_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_scale_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.05
  )

  # Modification: Non-uniform Scale Cube
  cube_xform = UsdGeom.Xformable.Get(stage, '/root/Cube')
  cube_xform.AddScaleOp(opSuffix='fuzz_scale').Set((1.2, 0.7, 1.5))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_scale_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_scale_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
