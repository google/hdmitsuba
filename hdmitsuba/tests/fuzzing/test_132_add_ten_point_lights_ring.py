...
"""Fuzz testing: dynamic multi-emitter ring spawning modification on USD stage."""
...
def test_add_ten_point_lights_ring():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_add_ten_point_lights_ring_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_add_ten_point_lights_ring_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Add 10 point lights in a circular ring
  radius = 3.0
  for i in range(10):
    theta = 2.0 * math.pi * i / 10.0
    x = radius * math.cos(theta)
    y = radius * math.sin(theta)
    light = UsdLux.SphereLight.Define(stage, f'/root/RingPoint_{i}')
    light.CreateTreatAsPointAttr(True)
    light.CreateIntensityAttr(10.0)
    light.CreateColorAttr(
        Gf.Vec3f(
            0.5 + 0.5 * math.cos(theta), 0.5 + 0.5 * math.sin(theta), 1.0
        )
    )
    light.AddTranslateOp().Set((x, y, 3.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_add_ten_point_lights_ring_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_add_ten_point_lights_ring_modified_usd.png'
  )
  test_helpers.robust_assert_close(
