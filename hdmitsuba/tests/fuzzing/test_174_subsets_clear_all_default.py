...
"""Fuzz testing: complete unbinding across all geometry subsets modification on USD stage."""
...
def test_subsets_clear_all_default():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_subsets_clear_all_default_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_subsets_clear_all_default_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: unbind all subset bindings
  ...
  UsdShade.MaterialBindingAPI(subset_def).UnbindAllBindings()
  UsdShade.MaterialBindingAPI(subset_red).UnbindAllBindings()
  UsdShade.MaterialBindingAPI(subset_blue).UnbindAllBindings()
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_subsets_clear_all_default_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_subsets_clear_all_default_modified_usd.png'
  )
  test_helpers.robust_assert_close(
