...
"""Fuzz testing: checkerboard colors swapping modification on USD stage."""
...
def test_checkerboard_swap():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_checkerboard_swap_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_checkerboard_swap_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: swap color0 and color1
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/ground/myshader/nested_bsdf/base_color'
  )
  c0 = shader.GetInput('color0').Get()
  c1 = shader.GetInput('color1').Get()
  shader.GetInput('color0').Set(c1)
  shader.GetInput('color1').Set(c0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_checkerboard_swap_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_checkerboard_swap_modified_usd.png'
  )
  test_helpers.robust_assert_close(
