...
"""Fuzz testing: UV transformation matrix modification on checkerboard BSDF in USD stage."""
...
def test_checkerboard_matrix_uv_scale():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_checkerboard_matrix_uv_scale_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_checkerboard_matrix_uv_scale_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: alter to_uv transformation matrix
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/ground/myshader/nested_bsdf/base_color'
  )
  mat = Gf.Matrix3d(10.0, 0, 0, 0, 10.0, 0, 0, 0, 1.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_checkerboard_matrix_uv_scale_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_checkerboard_matrix_uv_scale_modified_usd.png'
  )
  test_helpers.robust_assert_close(
