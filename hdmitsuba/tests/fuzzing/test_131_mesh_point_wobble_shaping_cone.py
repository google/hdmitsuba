...
"""Fuzz testing: concurrent point vibration and spot shaping cone oscillation modification on USD stage."""
...
def test_mesh_point_wobble_shaping_cone():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_mesh_point_wobble_shaping_cone_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_mesh_point_wobble_shaping_cone_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: wobble points and shift cone angle
  mesh = UsdGeom.Mesh.Get(stage, '/root/Plane/Plane')
  points = np.array(mesh.GetPointsAttr().Get())
  points[0] += np.array([0.0, 0.0, 0.5])
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_mesh_point_wobble_shaping_cone_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_mesh_point_wobble_shaping_cone_modified_usd.png',
  )
  test_helpers.robust_assert_close(
