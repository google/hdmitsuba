...
"""Fuzz testing: constant untextured dome light isolation modification under IrradianceMeter view on USD stage."""
...
def test_irradiancemeter_dome_isolation():
  ...
  # Disable original light and add untextured DomeLight
  orig_light = stage.GetPrimAtPath('/root/Light/Light')
  orig_light.SetActive(False)
  dome_light = UsdLux.DomeLight.Define(stage, '/root/IsoDome')
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_irradiancemeter_dome_isolation_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_irradiancemeter_dome_isolation_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: update dome intensity
  dome_light.GetIntensityAttr().Set(5.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_irradiancemeter_dome_isolation_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_irradiancemeter_dome_isolation_modified_usd.png',
  )
  test_helpers.robust_assert_close(
