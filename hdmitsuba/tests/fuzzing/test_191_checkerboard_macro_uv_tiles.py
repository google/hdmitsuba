...
"""Fuzz testing: macro tile multiplier transformation on checkerboard BSDF in USD stage."""
...
def test_checkerboard_macro_uv_tiles():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/materials/checkerboard.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_checkerboard_macro_uv_tiles_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_checkerboard_macro_uv_tiles_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: alter to_uv transformation matrix macro tiling
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/ground/myshader/nested_bsdf/base_color'
  )
  mat = Gf.Matrix3d(100.0, 0, 0, 0, 100.0, 0, 0, 0, 1.0)
  shader.GetInput('to_uv').Set(mat)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_checkerboard_macro_uv_tiles_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_checkerboard_macro_uv_tiles_modified_usd.png'
  )
  test_helpers.robust_assert_close(
