...
"""Fuzz testing: alternating points wobble modification on subdivision mesh in USD stage."""
...
def test_subdiv_points_wobble():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_subdiv_points_wobble_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_subdiv_points_wobble_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: wobble points
  mesh = UsdGeom.Mesh.Get(stage, '/root/Plane/Plane')
  points = np.array(mesh.GetPointsAttr().Get())
  points[0] += np.array([0, 0, 0.5])
  points[2] -= np.array([0, 0, 0.5])
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_subdiv_points_wobble_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_subdiv_points_wobble_modified_usd.png'
  )
  test_helpers.robust_assert_close(
