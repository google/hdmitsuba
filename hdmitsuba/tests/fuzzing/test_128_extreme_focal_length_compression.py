...
"""Fuzz testing: telephoto extreme focal length space compression modification on USD stage."""
...
def test_extreme_focal_length():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_extreme_focal_length_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_extreme_focal_length_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set telephoto focal length
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetFocalLengthAttr().Set(5.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_extreme_focal_length_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_extreme_focal_length_modified_usd.png'
  )
  test_helpers.robust_assert_close(
