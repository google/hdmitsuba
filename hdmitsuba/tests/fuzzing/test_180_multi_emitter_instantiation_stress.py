...
"""Fuzz testing: multi-instantiation dynamic RectLight grid spawning modification on USD stage."""
...
def test_multi_emitter_instantiation_stress():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/lights/area.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial,
      'test_multi_emitter_instantiation_stress_initial_hd.png',
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_multi_emitter_instantiation_stress_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Dynamic instantiation of 5 separate RectLight prims
  for i in range(5):
    rect = UsdLux.RectLight.Define(stage, f'/root/StressRect_{i}')
    rect.CreateWidthAttr(0.5)
    rect.CreateHeightAttr(0.5)
    rect.CreateIntensityAttr(5.0)
    rect.CreateColorAttr(Gf.Vec3f(0.2 * i, 0.8, 1.0 - 0.2 * i))
    rect.AddTranslateOp().Set((-2.0 + i * 1.0, 2.0, 2.0))
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_multi_emitter_instantiation_stress_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_multi_emitter_instantiation_stress_modified_usd.png',
  )
  test_helpers.robust_assert_close(
