...
"""Fuzz testing: geom subsets material modification on USD stage."""
...
def test_geom_subset_material_modification():
  ...
  test_helpers.write_image(
      image_hd_initial,
      'test_geom_subset_material_modification_initial_hd.png',
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_geom_subset_material_modification_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update diffuseColor on subset materials
  shader_red = UsdShade.Shader.Get(
      stage, '/root/_materials/red/Principled_BSDF'
  )
  shader_red.GetInput('diffuseColor').Set(Gf.Vec3f(0.1, 0.9, 0.2))

  shader_blue = UsdShade.Shader.Get(
      stage, '/root/_materials/blue/Principled_BSDF'
  )
  shader_blue.GetInput('diffuseColor').Set(Gf.Vec3f(0.9, 0.2, 0.9))
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_geom_subset_material_modification_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_geom_subset_material_modification_modified_usd.png',
  )
  test_helpers.robust_assert_close(
