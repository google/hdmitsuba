...
"""Fuzz testing: dynamic ShapingAPI application to point light in USD stage."""
...
def test_point_light_cone_attenuation():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/lights/point.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_point_light_cone_attenuation_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_point_light_cone_attenuation_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Apply ShapingAPI to pure point light
  light = UsdLux.SphereLight.Get(stage, '/root/Area/Area')
  shaping = UsdLux.ShapingAPI.Apply(light.GetPrim())
  shaping.CreateShapingConeAngleAttr(30.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_point_light_cone_attenuation_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_point_light_cone_attenuation_modified_usd.png'
  )
  test_helpers.robust_assert_close(
