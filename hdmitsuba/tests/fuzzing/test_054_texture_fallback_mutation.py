...
"""Fuzz testing: texture fallback color parameter modification on USD stage."""
...
def test_texture_fallback_modify():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_texture_fallback_modify_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_texture_fallback_modify_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update fallback color
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/Material/Texture_Fallback_Test'
  )
  shader.GetInput('fallback').Set(Gf.Vec4f(0.0, 0.0, 1.0, 1.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_texture_fallback_modify_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_texture_fallback_modify_modified_usd.png'
  )
  test_helpers.robust_assert_close(
