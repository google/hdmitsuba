...
"""Fuzz testing: degenerate bounding bounding box modification on subdivision geometry in USD stage."""
...
def test_subdiv_mesh_extent_degeneration():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_subdiv_mesh_extent_degeneration_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_subdiv_mesh_extent_degeneration_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set zero extent on subdiv mesh
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  mesh.GetExtentAttr().Set(
      Vt.Vec3fArray([Gf.Vec3f(0, 0, 0), Gf.Vec3f(0, 0, 0)])
  )
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_subdiv_mesh_extent_degeneration_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_subdiv_mesh_extent_degeneration_modified_usd.png',
  )
  test_helpers.robust_assert_close(
