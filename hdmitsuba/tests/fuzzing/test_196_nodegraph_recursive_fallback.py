...
"""Fuzz testing: recursive internal base color node unlinking inside NodeGraph hierarchy on USD stage."""
...
def test_nodegraph_recursive_fallback():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_nodegraph_recursive_fallback_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_nodegraph_recursive_fallback_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: unbind inner input connections inside encapsulation shader
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/encapsulation/NodeGraph/Shader'
  )
  shader.GetInput('diffuseColor').ClearSources()
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_nodegraph_recursive_fallback_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_nodegraph_recursive_fallback_modified_usd.png'
  )
  test_helpers.robust_assert_close(
