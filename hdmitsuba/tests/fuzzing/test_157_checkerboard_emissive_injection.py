...
"""Fuzz testing: emissive emission injection modification on checkerboard BSDF in USD stage."""
...
def test_checkerboard_emissive_injection():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_checkerboard_emissive_injection_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_checkerboard_emissive_injection_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Add emissiveColor to Principled_BSDF above checkerboard
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/ground/Principled_BSDF'
  )
  shader.CreateInput('emissiveColor', Sdf.ValueTypeNames.Color3f).Set(
      Gf.Vec3f(0.5, 0.5, 0.1)
  )
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_checkerboard_emissive_injection_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_checkerboard_emissive_injection_modified_usd.png',
  )
  test_helpers.robust_assert_close(
