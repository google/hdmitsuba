...
"""Fuzz testing: multi-mesh subdivision and spot light shaping modification on USD stage."""
...
def test_multimesh_subdiv_shaping():
  ...
  # Add a spot light
  spot_light = UsdLux.SphereLight.Define(stage, '/root/FuzzSpot')
  spot_light.CreateIntensityAttr(50.0)
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_multimesh_subdiv_shaping_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_multimesh_subdiv_shaping_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: scale points and change cone angle
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  points = np.array(mesh.GetPointsAttr().Get()) * 1.2
  mesh.GetPointsAttr().Set(Vt.Vec3fArray.FromNumpy(points))
  shaping.GetShapingConeAngleAttr().Set(20.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_multimesh_subdiv_shaping_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_multimesh_subdiv_shaping_modified_usd.png'
  )
  test_helpers.robust_assert_close(
