...
"""Fuzz testing: extreme non-uniform z-axis point scaling modification on BasisCurves in USD stage."""
...
def test_curves_nonuniform_extreme_z():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_curves_nonuniform_extreme_z_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_curves_nonuniform_extreme_z_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: scale points purely along z-axis by 10x
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  points = np.array(curves.GetPointsAttr().Get())
  points[:, 2] *= 10.0
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_curves_nonuniform_extreme_z_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_curves_nonuniform_extreme_z_modified_usd.png'
  )
  test_helpers.robust_assert_close(
