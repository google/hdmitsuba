...
"""Fuzz testing: dynamic geometry occlusion shift across IrradianceMeter view on USD stage."""
...
def test_irradiancemeter_dynamic_occlusion():
  ...
  test_helpers.write_image(
      image_hd_initial,
      'test_irradiancemeter_dynamic_occlusion_initial_hd.png',
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_irradiancemeter_dynamic_occlusion_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Translate Cube in front of sensor
  cube_xform = UsdGeom.Xformable.Get(stage, '/root/Cube')
  cube_xform.AddTranslateOp(opSuffix='fuzz_occl').Set((0.1, 0.1, 1.0))
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_irradiancemeter_dynamic_occlusion_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_irradiancemeter_dynamic_occlusion_modified_usd.png',
  )
  test_helpers.robust_assert_close(
