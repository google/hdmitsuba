...
"""Fuzz testing: dynamic terminal reconnection across dual outputs in NodeGraph on USD stage."""
...
def test_nodegraph_dual_output_swap():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_nodegraph_dual_output_swap_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_nodegraph_dual_output_swap_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: alter material connection on mitsuba_context material and rebind mesh to preview_input
  mat = UsdShade.Material.Get(stage, '/root/_materials/mitsuba_context')
  mat.GetOutput('mitsuba:surface').ClearSources()
  mat_other = UsdShade.Material.Get(stage, '/root/_materials/encapsulation')
  mesh = stage.GetPrimAtPath('/root/Meshes/Mesh4')
  UsdShade.MaterialBindingAPI(mesh).Bind(mat_other)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_nodegraph_dual_output_swap_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_nodegraph_dual_output_swap_modified_usd.png'
  )
  test_helpers.robust_assert_close(
