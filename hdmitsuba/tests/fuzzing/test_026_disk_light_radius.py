...
"""Fuzz testing: disk light radius modification on USD stage."""
...
def test_disk_light_radius():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_disk_light_radius_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_disk_light_radius_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update disk radius
  disk_light = UsdLux.DiskLight.Get(stage, '/root/Area/Area')
  disk_light.GetRadiusAttr().Set(0.8)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_disk_light_radius_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_disk_light_radius_modified_usd.png'
  )
  test_helpers.robust_assert_close(
