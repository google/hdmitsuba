...
"""Fuzz testing: dynamic addition of rect light to USD stage."""
...
def test_add_rect_light():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_add_rect_light_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_add_rect_light_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Define a RectLight
  rect_light = UsdLux.RectLight.Define(stage, '/root/NewRectLight')
  rect_light.CreateIntensityAttr(4.0)
  rect_light.CreateColorAttr(Gf.Vec3f(1.0, 0.2, 0.2))
  rect_light.CreateWidthAttr(2.0)
  rect_light.CreateHeightAttr(1.5)
  rect_light.AddTranslateOp().Set((0, -3, 3))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_add_rect_light_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_add_rect_light_modified_usd.png'
  )
  test_helpers.robust_assert_close(
