...
"""Fuzz testing: subdivision geometry modification on USD stage."""
...
def test_subdiv_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_subdiv_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_subdiv_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: scale points on subdiv mesh
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  points = np.array(mesh.GetPointsAttr().Get()) * 0.8
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_subdiv_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_subdiv_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
