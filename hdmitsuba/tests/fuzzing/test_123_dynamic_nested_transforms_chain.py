...
"""Fuzz testing: nested transformation chain modification across stage hierarchy on USD stage."""
...
def test_dynamic_nested_transforms_chain():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_dynamic_nested_transforms_chain_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_dynamic_nested_transforms_chain_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Chain transforms across parent /root/Cube and child /root/Cube/Cube
  parent_xform = UsdGeom.Xformable.Get(stage, '/root/Cube')
  parent_xform.AddRotateXYZOp(opSuffix='chain_p').Set((15, 0, 0))

  child_xform = UsdGeom.Xformable.Get(stage, '/root/Cube/Cube')
  child_xform.AddTranslateOp(opSuffix='chain_c').Set((0, 0.5, 0))
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_dynamic_nested_transforms_chain_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_dynamic_nested_transforms_chain_modified_usd.png',
  )
  test_helpers.robust_assert_close(
