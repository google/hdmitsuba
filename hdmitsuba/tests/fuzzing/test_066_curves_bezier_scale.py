...
"""Fuzz testing: curve points vector modification on USD stage."""
...
def test_curves_points_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_curves_points_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_curves_points_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: non-uniform scale on curve control points
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  points = np.array(curves.GetPointsAttr().Get()) * 1.5
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_curves_points_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_curves_points_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
