...
"""Fuzz testing: master compound fuzz combination across USD stage."""
...
def test_master_fuzz_combination():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_master_fuzz_combination_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_master_fuzz_combination_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: master compound changes across all aspects
  # 1. Curve widths
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  widths = np.array(curves.GetWidthsAttr().Get()) * 1.5
  curves.GetWidthsAttr().Set(Vt.FloatArray.FromNumpy(widths))

  # 2. Point light intensity
  point_light = UsdLux.SphereLight.Get(stage, '/root/Point')
  point_light.GetIntensityAttr().Set(100.0)

  # 3. Material color
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/Material_001/Principled_BSDF'
  )
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_master_fuzz_combination_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_master_fuzz_combination_modified_usd.png'
  )
  test_helpers.robust_assert_close(
