...
"""Fuzz testing: dynamic spawning of multiple overlapping meshes modification on USD stage."""
...
def test_add_three_overlapping_shapes():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_add_three_overlapping_shapes_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_add_three_overlapping_shapes_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Add 3 overlapping cubes
  orig = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  material = UsdShade.Material.Get(stage, '/root/_materials/Material')
  st_attr = orig.GetPrim().GetAttribute('primvars:st')

  for i, offset in enumerate([(0.5, 0, 0), (0, 0.5, 0), (0, 0, 0.5)]):
    cloned = UsdGeom.Mesh.Define(stage, f'/root/OverlapCube_{i}')
    cloned.GetPointsAttr().Set(orig.GetPointsAttr().Get())
    cloned.GetFaceVertexCountsAttr().Set(orig.GetFaceVertexCountsAttr().Get())
    cloned.GetFaceVertexIndicesAttr().Set(
        orig.GetFaceVertexIndicesAttr().Get()
    )
    cloned.GetExtentAttr().Set(orig.GetExtentAttr().Get())
    cloned.GetSubdivisionSchemeAttr().Set('none')
    if st_attr.IsValid():
      cloned.GetPrim().CreateAttribute(
          'primvars:st', st_attr.GetTypeName()
      ).Set(st_attr.Get())
    cloned.AddTranslateOp().Set(offset)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_add_three_overlapping_shapes_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_add_three_overlapping_shapes_modified_usd.png'
  )
  test_helpers.robust_assert_close(
