...
"""Fuzz testing: simultaneous translation shift and aspect ratio skew modification on USD stage."""
...
def test_camera_shift_aspect_ratio_skew():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_shift_aspect_ratio_skew_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_shift_aspect_ratio_skew_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Translate camera and skew aspect ratio
  cam_xform = UsdGeom.Xformable.Get(stage, '/root/Camera')
  cam_xform.AddTranslateOp(opSuffix='fuzz_skew').Set((0.2, 0.2, 0.0))
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetHorizontalApertureAttr().Set(2.0)
  camera.GetVerticalApertureAttr().Set(0.5)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_shift_aspect_ratio_skew_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_camera_shift_aspect_ratio_skew_modified_usd.png',
  )
  test_helpers.robust_assert_close(
