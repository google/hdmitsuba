...
"""Fuzz testing: multiple light compound modifications on USD stage."""
...
def test_multi_light_modifications():
  ...
  # Add Dome Light
  dome_light = UsdLux.DomeLight.Define(stage, '/root/DomeLight')
  dome_light.CreateIntensityAttr(0.2)

  # Add Distant Light
  dist_light = UsdLux.DistantLight.Define(stage, '/root/DistantLight')
  dist_light.CreateIntensityAttr(1.5)
  dist_light.AddRotateXYZOp().Set((30, 45, 0))
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_multi_light_modifications_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_multi_light_modifications_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: update multiple lights at once
  sphere_light = UsdLux.SphereLight.Get(stage, '/root/Light/Light')
  sphere_light.GetIntensityAttr().Set(500.0)
  dome_light.GetIntensityAttr().Set(0.5)
  dist_light.GetColorAttr().Set(Gf.Vec3f(0.9, 0.8, 0.2))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_multi_light_modifications_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_multi_light_modifications_modified_usd.png'
  )
  test_helpers.robust_assert_close(
