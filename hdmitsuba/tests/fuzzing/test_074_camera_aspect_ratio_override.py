...
"""Fuzz testing: camera aperture dimensions override on USD stage."""
...
def test_camera_aspect_override():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_aspect_override_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_aspect_override_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Non-uniform camera apertures
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetHorizontalApertureAttr().Set(0.5)
  camera.GetVerticalApertureAttr().Set(0.25)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_aspect_override_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_camera_aspect_override_modified_usd.png'
  )
  test_helpers.robust_assert_close(
