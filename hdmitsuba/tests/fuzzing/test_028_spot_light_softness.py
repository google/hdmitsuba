...
"""Fuzz testing: spot light shaping softness modification on USD stage."""
...
def test_spot_light_softness():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_spot_light_softness_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_spot_light_softness_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update shaping cone softness
  spot_light = UsdLux.SphereLight.Get(stage, '/root/Area/Area')
  shaping_api = UsdLux.ShapingAPI(spot_light.GetPrim())
  shaping_api.GetShapingConeSoftnessAttr().Set(0.8)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_spot_light_softness_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_spot_light_softness_modified_usd.png'
  )
  test_helpers.robust_assert_close(
