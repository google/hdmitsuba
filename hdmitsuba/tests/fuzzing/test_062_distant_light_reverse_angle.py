...
"""Fuzz testing: reverse rotation angle modification on DistantLight in USD stage."""
...
def test_distant_reverse_angle():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_distant_reverse_angle_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_distant_reverse_angle_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: reverse rotation angle
  light_xform = UsdGeom.Xformable.Get(stage, '/root/Area')
  light_xform.AddRotateXYZOp(opSuffix='fuzz_reverse').Set((180.0, 0.0, 0.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_distant_reverse_angle_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_distant_reverse_angle_modified_usd.png'
  )
  test_helpers.robust_assert_close(
