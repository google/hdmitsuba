...
"""Fuzz testing: extreme high index of refraction and zero specular modification on USD stage."""
...
def test_materials_ior_specular_variance():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_materials_ior_specular_variance_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_materials_ior_specular_variance_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set extreme IOR and zero specular
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/Material/Principled_BSDF'
  )
  shader.GetInput('ior').Set(2.5)
  shader.GetInput('specular').Set(0.0)
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_materials_ior_specular_variance_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_materials_ior_specular_variance_modified_usd.png',
  )
  test_helpers.robust_assert_close(
