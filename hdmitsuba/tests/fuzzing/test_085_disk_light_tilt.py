...
"""Fuzz testing: orientation tilt modification on DiskLight in USD stage."""
...
def test_disk_light_tilt():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_disk_light_tilt_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_disk_light_tilt_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Tilt disk orientation
  disk_xform = UsdGeom.Xformable.Get(stage, '/root/Area')
  disk_xform.AddRotateXYZOp(opSuffix='fuzz_tilt').Set((45.0, 30.0, 0.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_disk_light_tilt_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_disk_light_tilt_modified_usd.png'
  )
  test_helpers.robust_assert_close(
