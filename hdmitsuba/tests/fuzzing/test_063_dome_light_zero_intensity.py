...
"""Fuzz testing: zero intensity modification on DomeLight in USD stage."""
...
def test_dome_zero_intensity():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/lights/constant.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_dome_zero_intensity_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_dome_zero_intensity_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set dome light intensity to 0
  env_light = UsdLux.DomeLight.Get(stage, '/root/env_light')
  env_light.GetIntensityAttr().Set(0.0)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_dome_zero_intensity_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_dome_zero_intensity_modified_usd.png'
  )
  test_helpers.robust_assert_close(
