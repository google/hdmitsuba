...
"""Fuzz testing: sensor pixel filter type override modification on USD stage."""
...
def test_sensor_box_filter():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_sensor_box_filter_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_sensor_box_filter_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set pixel filter to box
  prim.GetAttribute('mitsuba:sensor:film:pixel_filter:type').Set('box')
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_sensor_box_filter_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_sensor_box_filter_modified_usd.png'
  )
  test_helpers.robust_assert_close(
