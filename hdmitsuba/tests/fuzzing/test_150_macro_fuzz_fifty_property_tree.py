...
"""Fuzz testing: massive multi-property compound tree permutation across USD stage."""
...
def test_macro_fuzz_fifty_property_tree():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_macro_fuzz_fifty_property_tree_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_macro_fuzz_fifty_property_tree_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Macro compound changes across stage hierarchy
  # 1. Mesh points and transforms
  mesh = UsdGeom.Mesh.Get(stage, '/root/Sphere_003/Sphere_003')
  points = np.array(mesh.GetPointsAttr().Get()) * 1.05
  mesh.GetPointsAttr().Set(Vt.Vec3fArray.FromNumpy(points))

  xform = UsdGeom.Xformable.Get(stage, '/root/Sphere_003')
  xform.AddTranslateOp(opSuffix='macro_t').Set((0.1, 0.1, 0.1))
  ...
  shader.GetInput('metallic').Set(0.5)

  # 3. Light properties
  light = UsdLux.SphereLight.Define(stage, '/root/MacroLight')
  light.CreateTreatAsPointAttr(True)
  light.CreateIntensityAttr(200.0)
  light.CreateColorAttr(Gf.Vec3f(0.8, 1.0, 0.8))
  light.AddTranslateOp().Set((0, 0, 4))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_macro_fuzz_fifty_property_tree_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_macro_fuzz_fifty_property_tree_modified_usd.png',
  )
  test_helpers.robust_assert_close(
