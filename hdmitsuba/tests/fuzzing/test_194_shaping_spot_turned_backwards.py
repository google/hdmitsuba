...
"""Fuzz testing: full 180 degree reversed orientation modification on SpotLight in USD stage."""
...
def test_shaping_spot_turned_backwards():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_shaping_spot_turned_backwards_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_shaping_spot_turned_backwards_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Rotate SpotLight to face exactly backward
  xform = UsdGeom.Xformable.Get(stage, '/root/Area')
  xform.AddRotateXYZOp(opSuffix='fuzz_back').Set((180, 0, 0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_shaping_spot_turned_backwards_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_shaping_spot_turned_backwards_modified_usd.png',
  )
  test_helpers.robust_assert_close(
