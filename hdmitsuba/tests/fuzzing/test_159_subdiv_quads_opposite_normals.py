...
"""Fuzz testing: anti-parallel quads explicit normal vector modification on subdivision mesh in USD stage."""
...
def test_subdiv_quads_opposite_normals():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/shapes/subdiv_two_quads.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_subdiv_quads_opposite_normals_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_subdiv_quads_opposite_normals_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set opposite normals on quads
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_subdiv_quads_opposite_normals_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_subdiv_quads_opposite_normals_modified_usd.png',
  )
  test_helpers.robust_assert_close(
