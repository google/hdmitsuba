...
"""Fuzz testing: one-frame material morph from rough diffuse to smooth metal mirror on USD stage."""
...
def test_diffuse_to_metal_mirror_swap():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_diffuse_to_metal_mirror_swap_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_diffuse_to_metal_mirror_swap_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: diffuse material transformed to smooth metal mirror
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_diffuse_to_metal_mirror_swap_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_diffuse_to_metal_mirror_swap_modified_usd.png'
  )
  test_helpers.robust_assert_close(
