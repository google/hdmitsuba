...
"""Fuzz testing: camera focal length modification on USD stage."""
...
def test_camera_focal_length():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_focal_length_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_focal_length_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update focalLength
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetFocalLengthAttr().Set(0.8)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_focal_length_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_camera_focal_length_modified_usd.png'
  )
  test_helpers.robust_assert_close(
