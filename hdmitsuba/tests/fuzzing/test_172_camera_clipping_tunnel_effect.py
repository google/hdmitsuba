...
"""Fuzz testing: tight clipping depth bounding around geometry modification on USD stage."""
...
def test_camera_clipping_tunnel():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_clipping_tunnel_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_clipping_tunnel_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Tight tunnel depth clipping
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetClippingRangeAttr().Set(Gf.Vec2f(4.0, 7.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_clipping_tunnel_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_camera_clipping_tunnel_modified_usd.png'
  )
  test_helpers.robust_assert_close(
