...
"""Fuzz testing: geometry extent attribute modification on USD stage."""
...
def test_geom_extent_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_geom_extent_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_geom_extent_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.05
  )

  # Modification: Update points and explicit extent
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  points = np.array(mesh.GetPointsAttr().Get()) * 0.7
  mesh.GetPointsAttr().Set(Vt.Vec3fArray.FromNumpy(points))
  mesh.GetExtentAttr().Set(
      Vt.Vec3fArray([Gf.Vec3f(-0.7, -0.7, -0.7), Gf.Vec3f(0.7, 0.7, 0.7)])
  )
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_geom_extent_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_geom_extent_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
