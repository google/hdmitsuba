...
"""Fuzz testing: interleaved varying widths modification on BasisCurves in USD stage."""
...
def test_multicurves_interleaved_width_mutate():
  ...
  test_helpers.write_image(
      image_hd_initial,
      'test_multicurves_interleaved_width_mutate_initial_hd.png',
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_multicurves_interleaved_width_mutate_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Interleaved width variation
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  widths = np.array(curves.GetWidthsAttr().Get())
  widths[::2] *= 3.0
  widths[1::2] *= 0.2
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_multicurves_interleaved_width_mutate_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_multicurves_interleaved_width_mutate_modified_usd.png',
  )
  test_helpers.robust_assert_close(
