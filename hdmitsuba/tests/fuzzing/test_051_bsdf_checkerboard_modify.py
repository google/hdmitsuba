...
"""Fuzz testing: checkerboard BSDF parameter modification on USD stage."""
...
def test_checkerboard_modify():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_checkerboard_modify_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_checkerboard_modify_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update checkerboard color0
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/ground/myshader/nested_bsdf/base_color'
  )
  shader.GetInput('color0').Set(Gf.Vec3f(0.1, 0.1, 0.9))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_checkerboard_modify_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_checkerboard_modify_modified_usd.png'
  )
  test_helpers.robust_assert_close(
