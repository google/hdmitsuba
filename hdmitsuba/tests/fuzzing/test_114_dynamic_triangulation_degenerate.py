...
"""Fuzz testing: collinear point assignment causing degenerate triangle generation on USD stage."""
...
def test_dynamic_triangulation_degenerate():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_dynamic_triangulation_degenerate_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_dynamic_triangulation_degenerate_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set collinear planar coordinates
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  points = np.array(mesh.GetPointsAttr().Get())
  points[:, 2] = 0.0  # Collapse along z axis entirely
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_dynamic_triangulation_degenerate_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_dynamic_triangulation_degenerate_modified_usd.png',
  )
  test_helpers.robust_assert_close(
