...
"""Fuzz testing: concurrent microfacet surface grid parameter permutation on USD stage."""
...
def test_clearcoat_roughness_specular():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_clearcoat_roughness_specular_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_clearcoat_roughness_specular_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Permute clearcoat, roughness, specular
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/Material/Principled_BSDF'
  )
  shader.GetInput('clearcoat').Set(0.9)
  shader.GetInput('roughness').Set(0.05)
  shader.GetInput('specular').Set(0.1)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_clearcoat_roughness_specular_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_clearcoat_roughness_specular_modified_usd.png'
  )
  test_helpers.robust_assert_close(
