...
"""Fuzz testing: material opacity modification on USD stage."""
...
def test_opacity_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_opacity_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_opacity_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set opacity
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/Material/Principled_BSDF'
  )
  shader.GetInput('opacity').Set(0.5)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_opacity_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_opacity_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
