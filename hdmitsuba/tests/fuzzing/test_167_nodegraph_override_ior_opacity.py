...
"""Fuzz testing: microfacet transmission opacity and IOR modification across NodeGraph hierarchy on USD stage."""
...
def test_nodegraph_override_ior_opacity():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_nodegraph_override_ior_opacity_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_nodegraph_override_ior_opacity_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Create and set IOR and opacity on preview_input
  shader = UsdShade.Shader.Get(stage, '/root/_materials/preview_input/Shader')
  shader.CreateInput('ior', Sdf.ValueTypeNames.Float).Set(1.9)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_nodegraph_override_ior_opacity_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_nodegraph_override_ior_opacity_modified_usd.png',
  )
  test_helpers.robust_assert_close(
