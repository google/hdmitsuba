...
"""Fuzz testing: material ior modification on USD stage."""
...
def test_ior_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_ior_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_ior_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set ior
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/Material/Principled_BSDF'
  )
  shader.GetInput('ior').Set(1.8)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_ior_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_ior_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
