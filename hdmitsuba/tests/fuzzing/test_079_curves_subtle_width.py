...
"""Fuzz testing: fine subtle width vector modification on BasisCurves in USD stage."""
...
def test_curves_subtle_width():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_curves_subtle_width_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_curves_subtle_width_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: fine subtle width
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  widths = np.array(curves.GetWidthsAttr().Get()) * 0.1
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_curves_subtle_width_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_curves_subtle_width_modified_usd.png'
  )
  test_helpers.robust_assert_close(
