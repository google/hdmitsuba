...
"""Fuzz testing: extreme asymmetrical aperture dimensions override on USD stage."""
...
def test_camera_asymmetrical_aperture():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_asymmetrical_aperture_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_asymmetrical_aperture_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Set extreme asymmetrical camera apertures
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetHorizontalApertureAttr().Set(10.0)
  camera.GetVerticalApertureAttr().Set(0.1)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_asymmetrical_aperture_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_camera_asymmetrical_aperture_modified_usd.png'
  )
  test_helpers.robust_assert_close(
