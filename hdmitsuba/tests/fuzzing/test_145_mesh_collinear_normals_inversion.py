...
"""Fuzz testing: explicit collinear point offset parallel to face normals on USD stage."""
...
def test_mesh_collinear_normals_inversion():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_mesh_collinear_normals_inversion_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial,
      'test_mesh_collinear_normals_inversion_initial_usd.png',
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Shift points parallel to normal vector
  mesh = UsdGeom.Mesh.Get(stage, '/root/Plane/Plane')
  points = np.array(mesh.GetPointsAttr().Get())
  points[:, 2] += 1.0  # translate along z normal
  ...
  test_helpers.write_image(
      image_hd_modified,
      'test_mesh_collinear_normals_inversion_modified_hd.png',
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_mesh_collinear_normals_inversion_modified_usd.png',
  )
  test_helpers.robust_assert_close(
