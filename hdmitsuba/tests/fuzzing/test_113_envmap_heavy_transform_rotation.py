...
"""Fuzz testing: intensive compound rotation matrix modification on Environment Map in USD stage."""
...
def test_envmap_heavy_transform():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_envmap_heavy_transform_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_envmap_heavy_transform_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: heavy multi-axis rotation on envmap
  xform = UsdGeom.Xformable.Get(stage, '/root/env_light')
  xform.AddRotateXYZOp(opSuffix='fuzz_heavy').Set((45.0, -60.0, 135.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_envmap_heavy_transform_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_envmap_heavy_transform_modified_usd.png'
  )
  test_helpers.robust_assert_close(
