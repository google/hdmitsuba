...
"""Fuzz testing: material unbinding modification on USD stage."""
...
def test_material_binding_clear():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_material_binding_clear_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_material_binding_clear_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: unbind material entirely
  cube_prim = stage.GetPrimAtPath('/root/Cube/Cube')
  UsdShade.MaterialBindingAPI(cube_prim).UnbindAllBindings()
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_material_binding_clear_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_material_binding_clear_modified_usd.png'
  )
  test_helpers.robust_assert_close(
