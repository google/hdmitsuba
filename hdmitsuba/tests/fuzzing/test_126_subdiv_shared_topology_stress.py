...
"""Fuzz testing: extreme geometry scaling and normals modification on subdivision mesh in USD stage."""
...
def test_subdiv_shared_topology_stress():
  ...
  engine.configure(
      hydra_delegate_id='HdMitsubaRendererPlugin', refine_level_fallback=3
  )

  image_hd_initial = engine.render()['color']
  scene_initial = mi.load_dict(
      usd_mitsuba.convert_to_mitsuba(stage, subdivision_level=3)
  )
  image_usd_initial = np.array(mi.render(scene_initial, spp=128))
  test_helpers.write_image(
      image_hd_initial, 'test_subdiv_shared_topology_stress_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_subdiv_shared_topology_stress_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Scale points by 3x
  mesh = UsdGeom.Mesh.Get(stage, '/root/Cube/Cube')
  points = np.array(mesh.GetPointsAttr().Get()) * 3.0
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_subdiv_shared_topology_stress_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified,
      'test_subdiv_shared_topology_stress_modified_usd.png',
  )
  test_helpers.robust_assert_close(
