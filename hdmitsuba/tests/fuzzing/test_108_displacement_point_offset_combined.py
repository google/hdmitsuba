...
"""Fuzz testing: simultaneous point displacement and microfacet material modification on USD stage."""
...
def test_displacement_point_offset():
  mi.set_variant('cuda_ad_rgb', 'llvm_ad_rgb')
  stage = Usd.Stage.Open(
      f'{test_helpers.TEST_ASSETS_PATH}/shapes/displacement_preview_surface.usda'
  )
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_displacement_point_offset_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_displacement_point_offset_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Shift points and update diffuseColor
  mesh = UsdGeom.Mesh.Get(stage, '/root/Plane/Plane')
  points = np.array(mesh.GetPointsAttr().Get()) * 1.5
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_displacement_point_offset_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_displacement_point_offset_modified_usd.png'
  )
  test_helpers.robust_assert_close(
