...
"""Fuzz testing: light color modification on USD stage."""
...
def test_light_color_modification():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_light_color_modification_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_light_color_modification_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.05
  )

  # Modification: Set light color
  light = UsdLux.SphereLight.Get(stage, '/root/Light/Light')
  light.GetColorAttr().Set(Gf.Vec3f(0.2, 0.5, 0.9))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_light_color_modification_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_light_color_modification_modified_usd.png'
  )
  test_helpers.robust_assert_close(
