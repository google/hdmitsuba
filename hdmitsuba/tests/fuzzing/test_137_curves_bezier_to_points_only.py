...
"""Fuzz testing: material unbinding and widths scaling modification on BasisCurves in USD stage."""
...
def test_curves_bezier_to_points_only():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_curves_bezier_to_points_only_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_curves_bezier_to_points_only_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: unbind material and scale widths
  curves = UsdGeom.BasisCurves.Get(stage, '/root/mycurve/curve')
  UsdShade.MaterialBindingAPI(curves.GetPrim()).UnbindAllBindings()
  widths = np.array(curves.GetWidthsAttr().Get()) * 0.05
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_curves_bezier_to_points_only_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_curves_bezier_to_points_only_modified_usd.png'
  )
  test_helpers.robust_assert_close(
