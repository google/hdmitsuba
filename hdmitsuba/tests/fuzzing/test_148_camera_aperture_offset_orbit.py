...
"""Fuzz testing: principal point orbital aperture offset modification on USD stage."""
...
def test_camera_aperture_offset_orbit():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_camera_aperture_offset_orbit_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_camera_aperture_offset_orbit_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Orbital aperture offsets
  camera = UsdGeom.Camera.Get(stage, '/root/Camera/Camera')
  camera.GetHorizontalApertureOffsetAttr().Set(0.15)
  camera.GetVerticalApertureOffsetAttr().Set(0.15)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_camera_aperture_offset_orbit_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_camera_aperture_offset_orbit_modified_usd.png'
  )
  test_helpers.robust_assert_close(
