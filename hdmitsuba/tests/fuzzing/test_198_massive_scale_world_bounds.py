...
"""Fuzz testing: global hierarchy root scaling modification across USD stage."""
...
def test_massive_scale_world():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_massive_scale_world_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_massive_scale_world_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: massive root scale
  root_xform = UsdGeom.Xformable.Get(stage, '/root')
  if root_xform:
    root_xform.AddScaleOp(opSuffix='fuzz_root').Set((20.0, 20.0, 20.0))
  else:
    cube_xform = UsdGeom.Xformable.Get(stage, '/root/Cube')
    cube_xform.AddScaleOp(opSuffix='fuzz_root').Set((20.0, 20.0, 20.0))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_massive_scale_world_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_massive_scale_world_modified_usd.png'
  )
  test_helpers.robust_assert_close(
