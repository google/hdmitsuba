...
"""Fuzz testing: nodegraph shader parameter modification on USD stage."""
...
def test_nodegraph_modify():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_nodegraph_modify_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_nodegraph_modify_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update color inside encapsulated nodegraph
  shader = UsdShade.Shader.Get(
      stage, '/root/_materials/encapsulation/NodeGraph/Shader'
  )
  shader.GetInput('diffuseColor').Set(Gf.Vec3f(0.9, 0.2, 0.8))
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_nodegraph_modify_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_nodegraph_modify_modified_usd.png'
  )
  test_helpers.robust_assert_close(
