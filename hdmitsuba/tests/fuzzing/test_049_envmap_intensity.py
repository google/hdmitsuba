...
"""Fuzz testing: envmap dome light intensity modification on USD stage."""
...
def test_envmap_intensity():
  ...
  test_helpers.write_image(
      image_hd_initial, 'test_envmap_intensity_initial_hd.png'
  )
  test_helpers.write_image(
      image_usd_initial, 'test_envmap_intensity_initial_usd.png'
  )
  test_helpers.robust_assert_close(
      image_hd_initial[..., :3], image_usd_initial, atol=0.2
  )

  # Modification: Update envmap intensity
  env_light = UsdLux.DomeLight.Get(stage, '/root/env_light')
  env_light.GetIntensityAttr().Set(3.5)
  ...
  test_helpers.write_image(
      image_hd_modified, 'test_envmap_intensity_modified_hd.png'
  )
  test_helpers.write_image(
      image_usd_modified, 'test_envmap_intensity_modified_usd.png'
  )
  test_helpers.robust_assert_close(
